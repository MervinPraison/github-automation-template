...
const { resolveGateConfigPath, hubScriptsDir } = require('./hub-paths.js');
const mergeGate = require(path.join(hubScriptsDir(), 'merge-gate.js'));
...
async function processWorkflowRunFailure(github, owner, repo, workflowRun, core) {
  ...
  const allowed = config.ciFailureWorkflowRuns || ['Core Tests', 'Optimized Test Suite'];
